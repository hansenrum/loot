$(function() {
  ...
  function update_airports_from_routes(routes) {
    ...
    _.each(routes, function(route) {
	    ...
	    // route as lines
	    var path = draw_route(airports_roundtrip , route.dest_host, airportMap );
	    ...
	    // var marker = svg.node().appendChild(ASSETS.star.cloneNode(true));
	    var marker = svg.append("path")
		.attr("class", "packet")
		.attr("d", MARKER_PATH)
		.attr("r", 5).attr("transform", "translate(" + startPoint + ")"+MARKER_CORRECTION);
	    transition(path, marker);
    });


    //var all_points = _.flatten(od_pairs),
    //    all_unique_points = _.unique(all_points, function(point){return "p"+point;});
    //draw_route(all_unique_points);
